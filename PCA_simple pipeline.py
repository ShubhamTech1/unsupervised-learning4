...
pw = 'grant3' # password
db = 'univ_db1' # database
# creating engine to connect database
engine = create_engine(f"mysql+pymysql://{user}:{pw}@localhost/{db}")
#conda install -c anaconda pymysql
# **Import the data**
University = pd.read_excel(r"D:\DATA SCIENTIST\DATA SCIENCE\STUDY MATERIAL\dimensionality reduction\PCA_code\University_Clustering.xlsx")

University

df = pd.read_excel(r"D:\DATA SCIENTIST\DATA SCIENCE\STUDY MATERIAL\dimensionality reduction\PCA_code\University_Clustering.xlsx")
...
# Define PCA model : principal component analysis

from sklearn.decomposition import PCA
pca = PCA(n_components = 6)                  # no. of columns = no.of pc's
...
# here our data is in standard format which name is univ
...
# weighted avg of every pcs : contribution of every column for each pcs.

# PCA weights
a = model['pca'].components_           # it gives weighted avg of every pcs
...
# then we calculate how much information stored in every pcs:
...
# then check how many pcs we want to extract which will give me more information means feature extraction:
...
newdf = pd.read_excel(r"D:\DATA SCIENTIST\DATA SCIENCE\STUDY MATERIAL\dimensionality reduction\PCA_code\University_Clustering.xlsx")
newdf
